...
| # | 作業 | いま置かれているサンプル値 |
|---|---|---|
| 1 | 事業者名を確定する | 「合同会社 ImagiCraft（サンプル）」 |
| 2 | 所在地を確定する | 「〒000-0000 東京都〇〇区〇〇1-2-3（サンプル）」 |
| 3 | 問い合わせメールを確定する | `support@usalingo.jp`（サンプル） |
| 4 | 施行日を確定する | 「2026年9月1日」「第1.0版」 |
| 5 | 同梱デッキ100語の出どころを確定する | credits.md の「（サンプル）基礎単語リスト NGSL」など |
| 6 | Bubble に `terms` / `privacy` / `credits` の3ページを作って公開する | 未作成 |
| 7 | 法的な適否の確認 | 未実施 |
| 8 | 実機で VoiceOver・Dynamic Type・リンク遷移を確認する | 未実施（USL-285 と同じやり方でできる） |

3〜5と6は所在地の公開可否と直結する。個人の自宅住所を公開したくない場合は、
所在地の書き方（バーチャルオフィス、または「請求があれば遅滞なく開示」）を先に決める必要がある。

### AI ができること（責任者が上を決めたあと）

| # | 作業 | 触る場所 |
|---|---|---|
| A | 確定値を3文書へ差し込み、「（サンプル）」の注記と警告ブロックを外す | `docs/legal/published/*.md` |
| B | 版・施行日をコードの定数へ反映する | `ProfileDashboardView.swift` の `publishedVersion` / `publishedEffectiveDate` |
| C | Bubble の実 URL に合わせて `publishedBaseURL` と slug を直す | 同上 `publishedBaseURL` |
| D | 問い合わせ先を1か所で合わせる | `Services/AppInfo.swift` の `supportEmail` |
| E | 同梱デッキの出典をクレジットへ書く | `docs/legal/published/credits.md` |
| F | Notion の実施レポート更新と done への変更 | Notion USL-255 |

コードは3か所の定数を直すだけで、新しい実装は要らない。

## 最短の順番

```text
所在地の書き方を決める（人間）
→ 事業者名・窓口・施行日・デッキ出典を確定（人間）
→ 3文書へ差し込み、コードの定数を合わせる（AI）
→ Bubble に3ページを公開（人間）
→ 実機でリンク遷移・VoiceOver・Dynamic Type を確認（人間）
→ USL-255 を done に（AI）
→ USL-287 TestFlight 提出へ
```

Bubble のページ公開より先にアプリ側の URL を決めておくと、slug の食い違いでリンクが切れる。
**先に slug を決め、その通りにページを作る**こと。
...
- コードは実装済みで `origin/main` に入っている。止まっているのは文書の実値と公開先だけ。
- USL-255 は M1 の最後の関門で、これが done になれば USL-287（TestFlight 提出）に進める。
- USL-278 の決定により、クレジットに書く素材は同梱デッキ `toeic-basic.json`（100枚、画像・音声なし）
  1本だけでよい。外部パッケージ依存はゼロ。
- Notion 上の worker_id は `codex-usl255-01a029` のまま残っているが lease は期限切れで、
  ブランチ `codex/usl-255-legal-credits` はリモートに存在しない。未マージの作業は無い。

やってほしいこと。

1. `docs/legal/published/` の3文書（terms-of-service / privacy-policy / credits）の
   サンプル値を一覧にして、責任者が答えるべき質問を最小の個数にまとめて聞く。
   所在地は自宅住所を公開したくない可能性があるので、書き方の選択肢を添えること。
2. 回答が出たら、3文書へ差し込み、`ProfileDashboardView.swift` の
   `publishedBaseURL` / `publishedVersion` / `publishedEffectiveDate` と
   `Services/AppInfo.swift` の `supportEmail` を合わせる。
3. Bubble のページ作成と実機確認は人間の作業なので、手順を小学生でもわかる比喩で
   端的に説明する。実機確認は USL-285 と同じやり方でよい。
