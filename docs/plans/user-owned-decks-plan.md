...
状態: migration作成済み・ローカル検証済み。本番Supabaseへは未適用、アプリ未実装
...
[ゲスト学習の引き継ぎ計画](guest-study-handoff-plan.md) の続きとして、
ログイン時に端末の独自デッキをアップロードする。

- `words` に一致する単語（`word_text` 一致）だけをリモートのデッキへ移す
- 一致しなかった単語は端末に残し、件数を画面に出す
- 黙って捨てない。捨てたことが分からない引き継ぎは作らない
...
| 2 | 本番Supabaseへ適用 | **未了**。2026-09-07 に承認を得たが、実行環境の権限設定でSupabaseへの書き込みと読み取りSQLが遮断された |
| 3 | アプリ実装 | **実装済み・未マージ**。ログイン中に追加・削除ができ、公式デッキは変わらない。段階2の完了までマージしない |
| 4 | ゲストからの引き継ぎ | ログイン時に独自デッキが移り、移せなかった件数が出る |
...
- ゲストの独自デッキがログイン時に移り、移せなかった件数が表示される
