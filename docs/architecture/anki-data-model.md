...
- 端末TTS、複数アルゴリズムは含めない。
- Deckには所有者（`decks.owner_id`）がある。`NULL` は運営が配る公式Deck、値があるときは
  その利用者だけの個人Deckとする。個人Deckは公式の単語を束ねるだけで、利用者が新しい
  単語を作ることは含めない。詳細は
  [個人デッキの計画](../plans/user-owned-decks-plan.md) を見る。
- Deckは世界観（`decks.concept_id` → `content_concepts`）を1つ持ち、どの例文・画像を
  出すかはDeckが決める。既定は `simple`。Cardに例文IDを持たせないのは、例文を作り直す
  たびにCardが変わり、進捗が孤児になるため。
