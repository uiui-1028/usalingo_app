> [!WARNING]
> これは複数アルゴリズムや旧MVP案を含む履歴資料です。現行仕様として使用しません。現行の正本は [Anki型学習コア仕様](../../architecture/anki-aligned-spec.md) と [Anki型データモデルと移行設計](../../architecture/anki-data-model.md) です。過去の比較・判断経緯を追えるよう本文を残しています。
