> [!WARNING]
> これは旧DB名を前提にした退会フローの履歴資料です。現行仕様として使用しません。現行資料の入口は [`docs/README.md`](../../README.md) です。退会仕様はUSL-254・USL-257の判断後に確定し、それまでは現行SwiftUIコードと実行可能migrationを事実の正本とします。過去案と削除順序の検討経緯を失わないため本文を残しています。
