...
- 判定: **合格**（2026-08-30 追記で更新。当初の判定は「停止のまま」だった）
...
| **隔離環境での検証** | 合格 | フルスタックのローカルSupabaseで、E2E 45件とData API公開範囲15件がすべて合格（下の追記） |

## 追記（2026-08-30）: 残っていた停止条件が解けた

人間のMacで、除外なしの `supabase start` によるフルスタック検証を実施した。

| 検証 | 結果 |
| --- | --- |
| `scripts/test-local-account-e2e.sh` | 45件すべて合格。失敗0件 |
| `scripts/test-local-data-api-exposure.sh` | 15件すべて合格。失敗0件 |
| 本番変更 | 0件 |

`20260830150000` と `20260830170000` は、本物のSupabaseスタック上で本物のroleに対して
適用に成功した。`anon` としてPostgREST経由で実際にリクエストした結果、
`asset_processing_queue`、Storage policyを作り直す2関数、監視系の関数とview、
公式コンテンツの2つのview、削除済みの `sync_existing_images` のすべてに到達できない。

この検証の途中で、`20260830150000` の取りこぼしを1件見つけて直した。
詳細は [USL-224](usl-224-close-production-exposure.md) の追記を参照。
カタログ上の権限だけを見るpgTAPでは検出できない種類の誤りであり、
Data APIを通す検証を別に持つ判断が正しかったことの実例である。

したがって解除条件4は満たされた。**この監査の判定を「合格」に変える。**

残るのは人間の承認だけである。USL-245 の本番適用には、対象project、適用対象migration、
バックアップ、切り戻し方法を提示した上での明示承認が必要である。
チケットを選んだだけでは本番migrationの実行許可にはならない。
